...
import simulationData from '../json/simulation-data.json';
...
export default function Dashboard() {
    // Simulated authenticated state – no Privy or real wallet dependency.
    ...
    useEffect(() => {
        ...
        // Drive the entire dashboard from local simulation data instead of backend APIs.
        const sim: any = simulationData;

        const simTrades: Trade[] = (sim.trades || []).slice(0, 5).map((t: any) => ({
            id: t.id,
        ...
        const simDeployments: Deployment[] = (sim.deployments || []).map((d: any) => ({
            id: d.id,
            agentId: d.agentId,
            agent: {
                name: d.agentName,
            ...
            status: d.status,
            sub_started_at: undefined,
        }));

        const openTrades = simTrades.filter((t) => t.status === 'OPEN' && t.unrealizedPnl);
        const totalUnrealizedPnl = openTrades.reduce((acc, t) => acc + parseFloat(t.unrealizedPnl || '0'), 0);
        const totalUnrealizedPnlPercent = openTrades.length
            ? openTrades.reduce((acc, t) => acc + parseFloat(t.unrealizedPnlPercent || '0'), 0) / openTrades.length
            : 0;

        setTrades(simTrades);
        setDeployments(simDeployments);
        setSummary({
        ...
        // Simple static simulated balances for demo mode.
        setBalance({
            usdc: '12500.50',
            eth: '3.42',
            credits: '5000',
