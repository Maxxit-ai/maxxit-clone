...
export default function Pricing() {
    // Simulated auth state – no Privy or real wallet dependency.
    const [authenticated, setAuthenticated] = useState(true);
    const login = () => setAuthenticated(true);
    const userWallet = UNIVERSAL_WALLET_ADDRESS;
    ...
    const handlePaymentSelection = (method: 'stripe' | 'web3') => {
        // Both Stripe and Web3 paths are now simulated locally – no backend/API calls.
        if (method === 'stripe') {
        ...
        <div className="min-h-screen bg-[var(--bg-deep)] text-[var(--text-primary)] font-mono">
            ...
            <main className="max-w-7xl mx-auto px-6 py-20">
                    ...
                    {isRedirecting && (
                        <div className="fixed inset-0 z-[110] bg-[var(--bg-deep)]/90 backdrop-blur-xl flex flex-center items-center justify-center flex-col gap-6 animate-in fade-in duration-500">
                            ...
                            <div className="text-center">
                                <h2 className="text-2xl font-display uppercase tracking-widest text-[var(--accent)] mb-2">SIMULATING STRIPE CHECKOUT</h2>
                                <p className="text-[var(--text-muted)] text-xs tracking-[0.2em] font-bold">NO REAL PAYMENT · DEMO MODE ONLY</p>
                            </div>
                        </div>
                    )}
                ...
                <Web3CheckoutModal
                    ...
                    onSuccess={(txHash) => {
                        console.log('Web3 Payment Success (simulated):', txHash);
                    }}
                />
