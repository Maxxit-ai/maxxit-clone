...
export function TradingPreferencesForm({
  ...
  useEffect(() => {
    // Removed API loading - using local state only
    setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Removed API saving - frontend-only mode
      console.log('[TradingPreferencesModal] Preferences saved (frontend-only):', preferences);
