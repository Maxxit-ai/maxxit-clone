...
export function TokenFilterSelector({ selectedTokens, onChange }: TokenFilterSelectorProps) {
    ...
    useEffect(() => {
        // Frontend-only: load available pairs from local simulation JSON
        try {
            setIsLoading(true);
            const { availablePairs } = simulationDataJson as any;
            
            if (availablePairs && availablePairs.tokens && Array.isArray(availablePairs.tokens)) {
                setTokens(availablePairs.tokens);
                setGroupedTokens(availablePairs.groupedTokens || {});
            } else {
                setError('No available pairs data found');
            }
        } catch (err: any) {
